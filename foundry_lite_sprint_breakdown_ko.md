...
- Sprint 46 이후 post-MVP 확장 순서는 [Data Platform Expansion Sprint Plan](./docs/data-platform-expansion-sprint-plan-ko.md)을 따른다. 현재는 S46 Semantic SSOT/Data Pattern Matrix가 complete이고, S47~S58C/S60~S63은 각 문서와 evidence ledger에 적힌 범위 안에서 partial/current proof를 갖는다. S61은 generated SDK와 frontend/backend surface lock, 43 route-surface request contract, 12 helper-surface contract, S62는 Dataset Explorer backend/API/SDK 시작점, S63은 Insight Review queue backend/API/SDK slice까지를 뜻하며 full visual workspace는 future scope다.
