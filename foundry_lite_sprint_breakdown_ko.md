...
- Sprint 46 이후 post-MVP 확장 순서는 [Data Platform Expansion Sprint Plan](./docs/data-platform-expansion-sprint-plan-ko.md)을 따른다. 현재는 S46 Semantic SSOT/Data Pattern Matrix가 complete이고, S47~S58C/S60~S64는 각 문서와 evidence ledger에 적힌 범위 안에서 partial/current proof를 갖는다. S61은 generated SDK와 Ontology-style OSDK facade, TypeScript ObjectSet property-keyed `where(...)`/chainable `orderBy(...)`/`aggregate(...)` `$count` exact-groupBy/`$pageSize` alias, Python OSDK mirror의 같은 `$count` exact-groupBy, Developer Console-lite OSDK app/scope/client registry, local OAuth authorization-code/PKCE/token/refresh/revoke proof, local TS/Python SDK release artifacts/channels/compatibility windows/download tokens, ObjectSet WebSocket `.subscribe(...)` with SSE fallback, session token provider, bounded operation polling, fetch-based operation event streaming helper, large ontology registry lookup/live-catalog search/action grouping/dynamic-only drift hint, Compass-style Projects/Resources DB/API/SDK surface, Pipeline Builder graph API/SDK surface, Source onboarding recipe/hook, Source Wizard recipe/hook, Generic REST connector onboarding recipe/hook, durable Source/Sync 분리와 감사 가능한 disable/re-enable, Source scheduler schedule edit/pause/resume, 연속 실패 자동 일시정지, 복구 빌드 성공 후 자동 재개, preview/tick API/SDK와 실패 진단 UI, admin readiness overview/screen/preflight/task-plan/operations-board model, bounded outbox publish admin start, frontend/backend surface lock, 269 route-surface request contract, 28 helper-surface contract, S62는 Dataset Explorer backend/API/SDK 시작점, S63은 Insight Review queue backend/API/SDK slice, S64는 Operations Recovery overview와 post-restore validation backend/API/SDK slice까지를 뜻하며 full visual workspace, folder/resource direct grant override, SAP/NetSuite/OAuth packaged source wizard, external NPM/PyPI publishing, external IdP introspection/refresh-token revocation, exactly-once subscription delivery, direct migration/worker/bootstrap browser control은 future scope다.
- S61 OSDK facade의 현재 범위에는 TypeScript object instance의 첫 relation DSL과 Python application OSDK mirror 1차도 포함된다. 즉 `order.$link.customer.fetchPage(...)`, `order.$actions.approveOrder.validateAction(params)`, `order.$actions.approveOrder.applyAction(params, { idempotencyKey, onCacheRefresh })`, `foundry(Order).where(status={"eq": "PENDING"}).fetch_page(...)`, `foundry(ApproveOrder).validate_action(...)`, `foundry(ApproveOrder).apply_action(..., idempotency_key=...)`, TS/Python `$count` exact-groupBy `aggregate(...)`, TS/Python SDK package manifest와 live-catalog drift report/assert gate, Developer Console-lite app/resource/client grants, JWT claim 기반 app scope enforcement, local SDK version/artifact registry lifecycle, local OAuth/session lifecycle, and WebSocket/SSE ObjectSet subscription proof are current. Richer aggregate functions, multi-hop/batched link traversal, full visual action validation/cache refresh UX, visual Developer Console/login/session UI, external registry publishing, external IdP introspection/refresh-token revocation, exactly-once subscription delivery, and Palantir-grade external package lifecycle/deployment compatibility windows remain future scope.
...
초기 MVP가 작더라도 나중에 팔란티어급 대용량 데이터, Spark/Kafka/S3/Iceberg/Elasticsearch와 자체 지속 실행 streaming worker 같은 인프라로 확장할 수 있게 core 제품 로직과 concrete infrastructure를 분리한다. 이 스프린트의 목표는 대형 인프라를 지금 모두 붙이는 것이 아니라, 나중에 갈아끼울 때 유지보수 비용과 추적 손실이 폭발하지 않도록 port/interface, adapter contract, composition root, trace key, contract test를 먼저 고정하는 것이다.
...
- [x] Source UI의 local SQL/REST `testConnection`은 현재 구현했다. Source config fingerprint와 `Idempotency-Key`를 검증하고 실제 REST preview 또는 SQL `SELECT 1`/resource inspection을 실행하되 Dataset version은 만들지 않으며, 단계별 결과·redacted error·request ID를 영구 이력으로 보존한다. production PostgreSQL driver/network, real Agent proxy, TLS/driver terminal은 future다. ([Implementation Status](./docs/implementation-status.md), [VERIFY-SOURCE-CONNECTION-DIAGNOSTIC-UI](./docs/sprint-evidence-ledger.md#verify-source-connection-diagnostic-ui))
...
- [x] production lease/fencing/Kubernetes scheduler packaging과 managed Temporal transform scheduling은 여전히 future/deferred다. 다만 current Temporal adapter/control-plane proof와 별개로, `foundry.transforms.run_graph(...)` bounded entrypoint는 현재 output dataset을 입력으로 선언한 downstream transform을 자동 실행하고 audit/run relation evidence를 남기며, `foundry.transforms.preview_due(...)`/`run_due(...)` bounded scheduler tick과 `worker:transform-scheduler`는 snapshot transform의 최신 입력 dataset version 변화만 감지해 재빌드하고 worker JSON evidence를 남긴다. Append/incremental transform 자동 tick은 duplicate-safe merge policy 전까지 fail-closed다. (`tests/integration/test_closed_loop.py::test_transform_graph_runs_downstream_transform_and_records_relation`, `tests/integration/test_closed_loop.py::test_transform_scheduler_tick_rebuilds_snapshot_when_input_version_changes`, `tests/unit/test_transform_scheduler_worker.py`)
...
- [x] Python runner는 `PythonTransformPlan`과 `foundry_lite.transforms_sdk` handle을 통해 실행된다. `tests/integration/test_closed_loop.py::test_python_transform_runs_through_sdk_handles_without_raw_storage_path`가 SDK input/output handle, output commit, definition snapshot source hash를 검증한다.
- [x] Python transform lineage/output commit은 SQL transform과 같은 Dataset transaction/finalization path를 사용한다. Full process sandbox와 외부 worker isolation은 아직 future scope다.
- [x] Python transform 실패 transaction path는 기존 transform abort/fail-run path를 재사용한다. `tests/integration/test_closed_loop.py`의 transform failure cleanup proof와 `tests/contracts/test_compute_adapter_contract.py`가 compute adapter 계약을 함께 지킨다.
- [x] SDK 사용자 함수에는 raw committed storage path 대신 `Input.read_rows/read_pandas/read_polars`, `Output.write_rows/write_pandas/write_polars` handle이 전달된다. 직접 Python process sandbox는 future scope다.
- [x] Transform output mode는 `snapshot`과 `append`/`incremental`을 지원한다. `tests/integration/test_closed_loop.py::test_transform_append_output_mode_commits_append_transaction`가 APPEND dataset transaction으로 커밋되는지 검증한다.
...
현재 증명은 local/fake `StreamAdapter.publish_event` → `FoundryLiteCore.archive_stream_events` → `raw.shipment_events` append version 생성에 더해, 실패한 archive write가 offset cursor를 먼저 전진시키지 않는 regression까지 포함한다. `tests/integration/test_source_streaming_sync.py`는 두 번째 실행이 마지막 committed offset 다음 event만 읽고도 Dataset의 논리적 current view에서는 첫 APPEND와 두 번째 APPEND의 모든 행과 파일을 함께 제공하는지 검증한다. 또한 `tests/integration/test_kafka_live_broker_stream_archive.py`가 `KafkaContainer` live broker를 띄우고 `KafkaStreamAdapter.publish_event`로 실제 topic에 event를 넣은 뒤 `foundry_lite_worker.stream_archive.run_stream_archive_once`가 같은 `archive_stream_events` application boundary를 통해 raw archive dataset version을 commit하는 경로까지 확인한다. 같은 live-broker suite의 Data Connection Source 시나리오는 broker connection test, topic metadata 탐색, preview-only key/value sample, managed streaming Sync, committed offset, broker high watermark/lag, Dataset preview를 하나의 사용자 흐름으로 검증한다.
...
- [x] SQL/DuckDB transform과 SDK-handle Python transform으로 clean dataset을 만든다. ([MVP-TRANSFORM](./docs/sprint-evidence-ledger.md#mvp-core-transform), `tests/integration/test_closed_loop.py::test_python_transform_runs_through_sdk_handles_without_raw_storage_path`)
