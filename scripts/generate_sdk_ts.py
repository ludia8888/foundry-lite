...
def render_typescript(ontology: OntologyDef) -> str:
    ...
    lines.extend(_client_type_lines(ontology))
    lines.extend(_client_runtime_lines(ontology))
    return "\n".join(lines) + "\n"
...
def render_web_javascript(ontology: OntologyDef) -> str:
    ...
    lines.extend(_web_object_client_lines(ontology.objects))
    lines.extend(["    },", "    actions: {"])
    lines.extend(_web_action_client_lines(ontology.actions))
    lines.extend(["    },", "  };", "}", ""])
    return "\n".join(lines)
...
def _client_type_lines(ontology: OntologyDef) -> list[str]:
    lines = ["export type FoundryLiteGeneratedClient = {", "  objects: {"]
    for object_def in ontology.objects:
        lines.extend(
            [
                f"    {object_def.api_name}: {{",
                f"      get(id: string, options?: {{ explain?: boolean }}): Promise<{object_def.api_name}>;",
                f"      query(payload?: ObjectQueryRequest): Promise<ObjectQueryResult<{object_def.api_name}>>;",
                "    };",
            ]
        )
    lines.extend(["  };", "  actions: {"])
    for action_def in ontology.actions:
        lines.extend(
            [
                ...
                f"      apply(payload: {action_def.api_name}ApplyRequest): Promise<ActionApplyResponse>;",
                "    };",
            ]
        )
    lines.extend(["  };", "};", ""])
...
def _client_runtime_lines(ontology: OntologyDef) -> list[str]:
    lines = [
    ...
    lines.extend(_ts_object_client_lines(ontology.objects))
    lines.extend(["    },", "    actions: {"])
    lines.extend(_ts_action_client_lines(ontology.actions))
    lines.extend(["    },", "  };", "}", ""])
    return lines


def _ts_object_client_lines(objects: Sequence[ObjectDef]) -> list[str]:
    lines: list[str] = []
    for object_def in objects:
        ...
        lines.extend(
            [
                ...
                (
                    f"          return request<{object_def.api_name}>(`/api/objects/{object_def.api_name}/"
                    "${encodeURIComponent(id)}${suffix}`);"
                ),
                "        },",
                "        query: (payload: ObjectQueryRequest = {}) =>",
                (
                    f"          request<ObjectQueryResult<{object_def.api_name}>>"
...
def _ts_action_client_lines(actions: Sequence[ActionDef]) -> list[str]:
    lines: list[str] = []
    for action_def in actions:
        ...
        lines.extend(
            [
                f"      {action_def.api_name}: {{",
                f"        apply: (payload: {action_def.api_name}ApplyRequest) =>",
...
def _web_object_client_lines(objects: Sequence[ObjectDef]) -> list[str]:
    lines: list[str] = []
    for object_def in objects:
        lines.extend(
            [
...
def _web_action_client_lines(actions: Sequence[ActionDef]) -> list[str]:
    lines: list[str] = []
    for action_def in actions:
        lines.extend(
            [
