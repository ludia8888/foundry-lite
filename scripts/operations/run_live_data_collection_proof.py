...
def run_live_data_collection_proof(config: LiveProofConfig) -> dict[str, object]:
    ...
    wiki_changes = _sync_wikipedia_recent_changes(foundry, ctx, config.limit)
    ...
    return {
        ...
        "sources": {
            ...
            "wikipediaRecentChanges": _commit_summary(wiki_changes, WIKI_BASE_URL),
...
def _sync_wikipedia_recent_changes(foundry: FoundryLite, ctx: RequestContext, limit: int) -> Any:
    query = urlencode(
        {
            "action": "query",
            "list": "recentchanges",
            "rcnamespace": "0",
            "rclimit": str(limit),
            "rcprop": "title|timestamp|ids|comment|user",
            "format": "json",
        }
    )
    return foundry.datasets.sync_connector_snapshot(
        ...
        connector_name="rest",
        resource_name="wikipedia_recentchanges",
        ctx=ctx,
        rest=RestSourceConfig(
            base_url=WIKI_BASE_URL,
            resource_path=f"/w/api.php?{query}",
            auth=_user_agent_header(),
            pagination=RestPaginationConfig(
                items_path="query.recentchanges",
                next_cursor_path="continue.rccontinue",
                cursor_query_param="rccontinue",
                cursor_key="rccontinue",
            ),
            schema_columns=("rcid", "pageid", "revid", "old_revid", "title", "timestamp", "user", "comment", "type"),
        ),
    )
...
def _load_json_url(url: str) -> object:
    ...
    with urlopen(request, timeout=10) as response:  # noqa: S310 - live proof intentionally calls public HTTPS APIs.
