...
def test_rest_connector_snapshot_commits_raw_dataset(tmp_path) -> None:
    with MockRestServer() as server:
        foundry = _core_with_rest_connector(tmp_path)
        ctx = demo_admin_context()
        foundry.datasets.ensure("raw.rest_orders", ctx=ctx, primary_key=["order_id"])
    ...
    preview = foundry.datasets.preview("raw.rest_orders", ctx=ctx)
    runs = foundry.operations.query_runs(ctx=ctx, run_type="sync", status="COMMITTED")
...
def test_rest_connector_snapshot_resumes_from_committed_cursor(tmp_path) -> None:
    with MockRestServer() as server:
        ...
        first = foundry.datasets.sync_connector_snapshot(
            "raw.rest_orders",
            connector_name="rest",
            resource_name="orders",
            ctx=ctx,
        ...
        second = foundry.datasets.sync_connector_snapshot(
            "raw.rest_orders",
            connector_name="rest",
            resource_name="orders",
            ctx=ctx,
    ...
    latest_metadata = _transaction_metadata(foundry, second.transaction_id)
    preview = foundry.datasets.preview("raw.rest_orders", ctx=ctx)
...
def test_rest_cursor_not_advanced_when_dataset_commit_fails(tmp_path) -> None:
    with MockRestServer() as server:
        ...
        foundry.datasets.sync_connector_snapshot(
            "raw.rest_orders",
            connector_name="rest",
            resource_name="orders",
            ctx=ctx,
    ...
    latest_metadata = _transaction_metadata(foundry, retry.transaction_id)
...
def test_rest_connector_rate_limit_failure_is_visible_in_operations(tmp_path) -> None:
    with MockRestServer() as server:
        foundry = _core_with_rest_connector(tmp_path)
        ctx = demo_admin_context()
        foundry.datasets.ensure("raw.rest_limited", ctx=ctx, primary_key=["order_id"])
...
def _transaction_metadata(foundry: FoundryLite, transaction_id: str) -> dict[str, object]:
