...
def test_builder_mcp_exposes_chatgpt_domain_os_studio_and_plans_korean_business_name(
    ...
    assert "extract every business record, actor, lifecycle step, action, rule" in instructions
    assert "in the user's language" in instructions
    ...
    assert 'state.loadStatus = "unavailable"' in html
    assert "이 카드에는 업무 설계가 없습니다" in html
