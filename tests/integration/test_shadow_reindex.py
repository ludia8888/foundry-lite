...
def test_shadow_reindex_replays_action_edits(
    ...
    result = core.objects.shadow_reindex("Order", ctx=ctx)
    after_switch = core.objects.get("Order", "O-1001", ctx=ctx)
    query_page = core.objects.query("Order", ctx=ctx, limit=10)
...
def test_shadow_reindex_alias_switch_cursor_version_safe(
    ...
    first_page = core.objects.query("Order", ctx=ctx, order_by=order_by, limit=1)
    ...
    with pytest.raises(ValidationFailed, match="active index version"):
        core.objects.query("Order", ctx=ctx, order_by=order_by, cursor=cursor)
    assert result["is_switched"] is True
    fresh_page = core.objects.query("Order", ctx=ctx, order_by=order_by, limit=3)
...
def test_shadow_reindex_validation_failure_keeps_existing_active_index(
    ...
    ctx = prepare_indexed_demo(core)
    before = core.objects.get("Order", "O-1001", ctx=ctx)

    with pytest.raises(ValidationFailed):
        core.objects.shadow_reindex("Order", ctx=ctx, expected_hash="not-the-baseline-hash")

    after_failure = core.objects.get("Order", "O-1001", ctx=ctx)
    query_page = core.objects.query("Order", ctx=ctx, limit=10)
    failed_runs = [
        run for run in core.operations.list_runs(ctx=ctx)["indexRuns"] if run["trigger_type"] == "shadow_reindex"
    ]
...
def test_reindex_same_dataset_version_idempotent(core: FoundryLite) -> None:
    ctx = prepare_indexed_demo(core)
    before = core.objects.get("Order", "O-1001", ctx=ctx)
    changed_before = _object_changed_count(core, ctx, "Order/O-1001")

    result = core.objects.reindex("Order", ctx=ctx)
...
def test_reindex_same_dataset_version_does_not_break_expected_object_version(core: FoundryLite) -> None:
    ctx = prepare_indexed_demo(core)
    order = core.objects.get("Order", "O-1001", ctx=ctx)
...
def test_object_version_increments_for_base_and_edit_updates(core: FoundryLite, tmp_path: Path) -> None:
    ctx = prepare_indexed_demo(core)
    base_order = core.objects.get("Order", "O-1001", ctx=ctx)
    ...
    reindexed = core.objects.get("Order", "O-1001", ctx=ctx)
...
def test_object_merge_edit_only_not_overwritten_by_source(core: FoundryLite, tmp_path: Path) -> None:
    ctx = prepare_indexed_demo(core)
    base_order = core.objects.get("Order", "O-1001", ctx=ctx)
...
def test_shadow_reindex_catches_up_delta_edits_before_switch(
    core: FoundryLite, monkeypatch: pytest.MonkeyPatch
) -> None:
    ctx = prepare_indexed_demo(core)
    order = core.objects.get("Order", "O-1001", ctx=ctx)
...
def test_index_progress_cursor_advances_only_after_bulk_upsert_commit(
    ...
    failed_run = next(
        run
        for run in core.operations.list_runs(ctx=ctx)["indexRuns"]
...
def _approve_with_note(core: FoundryLite, ctx: RequestContext, expected_object_version: int, note: str) -> RuntimeRow:
    core.actions.apply(
        "ApproveOrder",
        object_type="Order",
        object_id="O-1001",
    ...
    return core.objects.get("Order", "O-1001", ctx=ctx)


def _reindex_with_changed_amount(core: FoundryLite, tmp_path: Path, ctx: RequestContext, *, amount: str) -> None:
    ...
    core.datasets.upload_csv("raw.erp_orders", str(modified_csv), ctx=ctx)
    core.transforms.run("clean_orders", ctx=ctx)
    core.objects.reindex("Order", ctx=ctx)


def _object_changed_count(core: FoundryLite, ctx: RequestContext, aggregate_id: str) -> int:
    return sum(
        1
...
def _index_run(core: FoundryLite, ctx: RequestContext, run_id: str) -> RuntimeRow:
    return next(run for run in core.operations.list_runs(ctx=ctx)["indexRuns"] if run["id"] == run_id)
