...
def test_commit_dataset_version_aborts_when_primary_key_check_fails(
    ...
    with pytest.raises(ValidationFailed):
        core.datasets.upload_csv("raw.erp_orders", csv_path, ctx=ctx)

    runs = core.operations.list_runs(ctx=ctx)
    assert runs["syncRuns"][0]["status"] == "FAILED"
    assert core.datasets.list_versions("raw.erp_orders", ctx=ctx) == []
...
def test_dataset_health_check_reads_candidate_not_latest(
    ...
    ctx = demo_admin_context()
    core.datasets.ensure("raw.health_orders", ctx=ctx, primary_key=["order_id"])
    latest_csv = tmp_path / "latest_orders.csv"
    latest_csv.write_text("order_id,amount\nO-1,100\n", encoding="utf-8")
    committed = core.datasets.upload_csv("raw.health_orders", latest_csv, ctx=ctx)
    ...
    with pytest.raises(ValidationFailed, match="dataset checks failed") as exc_info:
        core.datasets.upload_csv("raw.health_orders", invalid_candidate, ctx=ctx)

    versions = core.datasets.list_versions("raw.health_orders", ctx=ctx)
    preview = core.datasets.preview("raw.health_orders", ctx=ctx)
