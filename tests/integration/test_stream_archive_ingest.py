...
def test_stream_archive_appends_preview_and_checkpoint_metadata(tmp_path: Path) -> None:
    ...
    core.datasets.ensure("raw.shipment_events", ctx=ctx, primary_key=["event_id"])
    _publish_shipment(dependencies.stream_adapter, "S-100", ctx=ctx)
    _publish_shipment(dependencies.stream_adapter, "S-101", ctx=ctx)

    result = core.datasets.archive_stream_events("raw.shipment_events", stream=stream, ctx=ctx)

    assert result is not None
    preview = core.datasets.preview("raw.shipment_events", ctx=ctx)
    latest = _latest_stream_transaction(core, dependencies, ctx, "raw.shipment_events")
    runs = core.operations.query_runs(ctx=ctx, run_type="sync", status="COMMITTED")["syncRuns"]
...
def test_stream_archive_restart_resumes_after_committed_offset(tmp_path: Path) -> None:
    ...
    first = core.datasets.archive_stream_events("raw.shipment_events", stream=stream, ctx=ctx)
    second = core.datasets.archive_stream_events("raw.shipment_events", stream=stream, ctx=ctx)

    assert first is not None
    assert second is not None
    latest_preview = core.datasets.preview("raw.shipment_events", ctx=ctx)
    latest = _latest_stream_transaction(core, dependencies, ctx, "raw.shipment_events")
...
def test_stream_offset_not_advanced_when_append_commit_fails(tmp_path: Path) -> None:
    core, dependencies = _core_with_stream(tmp_path)
    failing_core = FoundryLite(dependencies=replace(dependencies, compute_adapter=_ExplodingRowsComputeAdapter()))
    ctx = demo_admin_context()
    ...
    with pytest.raises(ValidationFailed, match="stream archive failed"):
        failing_core.datasets.archive_stream_events("raw.shipment_events", stream=stream, ctx=ctx)
    retry = core.datasets.archive_stream_events("raw.shipment_events", stream=stream, ctx=ctx)
...
def test_stream_archive_failure_is_visible_in_operations(tmp_path: Path) -> None:
    ...
    core = FoundryLite(dependencies=replace(dependencies, compute_adapter=_ExplodingRowsComputeAdapter()))
    ctx = demo_admin_context()
    stream = StreamArchiveConfig(stream_name="shipments", topic="shipment_events")
    core.datasets.ensure("raw.shipment_events", ctx=ctx, primary_key=["event_id"])
    _publish_shipment(dependencies.stream_adapter, "S-100", ctx=ctx)
    ...
    with pytest.raises(ValidationFailed, match="stream archive failed"):
        core.datasets.archive_stream_events("raw.shipment_events", stream=stream, ctx=ctx)
...
def _latest_stream_transaction(
    ...
    dataset = core.datasets.get(dataset_ref, ctx=ctx)
