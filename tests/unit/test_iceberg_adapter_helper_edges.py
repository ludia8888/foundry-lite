...
def test_iceberg_sidecar_manifest_and_pruning_helper_edges(tmp_path) -> None:
    manifest_file: DatasetManifestFile = {
        ...
        "row_count": 2,
        "byte_size": 4,
        "content_hash": ice._path_sha256(_write_bytes(tmp_path / "orders-a.parquet", b"data")),
    ...
    expected = ice._expected_files(raw_sidecar)
    ...
    ice._copy_expected_manifest_metadata(copied, expected[manifest_file["uri"]])

    assert expected[manifest_file["uri"]]["column_stats"]["amount"]["null_count"] == 0
    assert copied["partition_values"] == {"region": "NA"}
    assert ice._matching_expected_files(expected, None) == [expected[manifest_file["uri"]]]
    assert ice._matching_expected_files(expected, {"region": "NA"}) == [expected[manifest_file["uri"]]]
    assert ice._matching_expected_files(expected, {"amount": 15}) == [expected[manifest_file["uri"]]]
    assert ice._matching_expected_files(expected, {"amount": 999}) == []
    assert ice._file_might_match_value(
        {"uri": "x", "format": "parquet", "row_count": 1, "byte_size": 1, "content_hash": "h"}, "missing", "v"
    )
    with pytest.raises(ValueError, match="file list"):
        ice._expected_files({"files": "not-a-list"})
    with pytest.raises(ValueError, match="malformed file entry"):
        ice._expected_files({"files": ["bad"]})
    with pytest.raises(ValueError, match="byte size mismatch"):
        ice._verify_downloaded_snapshot_file(tmp_path / "orders-a.parquet", {**manifest_file, "byte_size": 99})
    with pytest.raises(ValueError, match="content hash mismatch"):
        ice._verify_downloaded_snapshot_file(tmp_path / "orders-a.parquet", {**manifest_file, "content_hash": "bad"})
...
def test_iceberg_row_count_maintenance_and_schema_helper_edges(tmp_path) -> None:
    ...
    plan = ice._maintenance_plan("iceberg", "raw.orders", "dataset-1", "main", "ns.table", policy, snapshots, current)
    rewrite = ice._rewrite_result(1, 3, 5, 1, "before-hash", "before-hash")

    assert ice._part_row_count(DatasetStagedFile(path=tmp_path / "part.parquet", row_count=None), 7) == 7
    assert ice._staged_files_row_count((DatasetStagedFile(path=tmp_path / "a.parquet", row_count=2),), 7) == 2
    assert ice._uploaded_row_count(DatasetStagedFile(path=tmp_path / "a.parquet", row_count=2), 2) == 2
    with pytest.raises(ValueError, match="row count does not match"):
        ice._uploaded_row_count(DatasetStagedFile(path=tmp_path / "a.parquet", row_count=2), 3)
    ...
    assert ice._current_compaction_candidate(plan)["snapshot_id"] == 2
    assert ice._maintenance_run_status(ice._empty_rewrite_result(None), [], "no_table") == "no_table"
    assert ice._maintenance_run_status(ice._empty_rewrite_result(None), [], "healthy") == "skipped"
    assert ice._maintenance_run_status(rewrite, [1], "maintenance_needed") == "completed"
    assert ice._maintenance_reason_codes(2, 10, None, policy) == [
        "orphan_snapshot",
        "too_many_files",
        "small_files",
        "read_amplification",
    ]
    assert ice._committed_snapshot_versions(
...
def test_iceberg_compatible_arrow_type_edges() -> None:
    ...
    converted = ice._iceberg_compatible_arrow_table(table)
    ...
    assert (
        ice._snapshot_token([{"uri": "a", "format": "parquet", "row_count": 1, "byte_size": 1, "content_hash": "h1"}])
        == "h1"
    )
    assert (
        len(
