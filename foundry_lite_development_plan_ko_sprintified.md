...
- [ ] 각 boundary는 `Protocol` 또는 명시적 interface로 정의되고, application service는 concrete SDK가 아니라 이 boundary를 호출한다.
  ...
  - [x] `DatasetRepository`는 dataset registry create/find/version-list DB read/write 경계를 맡고 local/fake contract test를 통과한다.
  - [ ] `DatasetTransactionRepository`, `ComputeAdapter`, `StreamAdapter`, `SearchAdapter`, `WorkflowAdapter`, `ConnectorAdapter`, `AuthProvider`는 아직 application service에서 완전히 포트화되지 않았다.
...
- [ ] CI는 domain/application이 금지된 concrete infra SDK를 직접 import하면 실패한다.
  - [x] 현재 CI는 domain concrete infra import 0개, application concrete infra import baseline 32개 초과, scale SDK 직접 import, mixin method 충돌을 잡는다.
  - [x] application concrete infra import baseline을 37에서 32로 낮췄다.
  - [ ] application concrete infra import baseline을 32에서 0으로 낮추는 repository/adapter 추출은 남아 있다.
