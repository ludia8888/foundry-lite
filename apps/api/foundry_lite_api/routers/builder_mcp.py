...
def _initialize_result(protocol_version: str) -> dict[str, object]:
    return {
        ...
        "instructions": (
            ...
            "Before that one read-only call, extract every business record, actor, lifecycle step, action, rule, "
            "required input, and evidence item explicitly stated by the user. Preserve their business-facing names "
            "in the user's language, including applicationName, record names, actor names, lifecycleStates, and "
            "action names; reserve English identifiers for optional apiName fields only. Do not omit stated records "
            "or rules merely because they are not technically required by the schema. Ensure every action "
            "fromStates/toState matches lifecycleStates exactly and every allowedActor matches actors exactly. "
            "Leave genuinely uncertain business details for readiness questions rather than guessing. "
            ...
            "project:<project-id> for app generation. "
            ...
            "and evidence, then explicitly create a test application. The plan tool also advertises an embedded "
            "MCP App review card: do not claim that no interactive card appeared merely because the tool data is JSON; "
            "if host rendering cannot be observed, say it is unverified. Use lazy discovery with search_tools or eager "
