...
def _builder_mcp_post_response(
    ...
    try:
        ...
        result = _dispatch(application_id, session_id, payload, negotiated_protocol, ctx)
    except FoundryLiteError as exc:
...
def _dispatch(
    ...
    if method == "tools/call":
        return _call_tool(application_id, session_id, payload, ctx)
    _require_active_session(application_id, session_id, ctx)
...
def _initialize_result(protocol_version: str) -> dict[str, object]:
    return {
        ...
        "instructions": (
            ...
            "Mutations return an approval challenge; a human must approve it through the authenticated "
            "control-plane endpoint before the one-time confirmationReceipt can be retried. "
...
def _required_existing_session_id(request: Request) -> str:
    return require_mcp_session_id(request, "Builder")
