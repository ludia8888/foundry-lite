...
class TransformSchedulerWorkerConfig:
    storage_root: Path
    ...
    actor_user_id: str = "worker-transform-scheduler"
    request_id: str = "req-worker-transform-scheduler"
...
@dataclass(frozen=True)
class TransformSchedulerWorkerResult:
...
def config_from_env(env: Mapping[str, str] | None = None) -> TransformSchedulerWorkerConfig:
    values = env or os.environ
    return TransformSchedulerWorkerConfig(
        ...
        max_runs_per_tick=_env_int(values, "FOUNDRY_LITE_TRANSFORM_SCHEDULER_MAX_RUNS", 50),
        max_ticks=_env_int(values, "FOUNDRY_LITE_TRANSFORM_SCHEDULER_MAX_TICKS", 0),
        max_empty_ticks=_env_int(values, "FOUNDRY_LITE_TRANSFORM_SCHEDULER_MAX_EMPTY_TICKS", 0),
        tick_interval_seconds=_env_float(values, "FOUNDRY_LITE_TRANSFORM_SCHEDULER_INTERVAL_SECONDS", 60.0),
        is_continuous=_env_bool(values, "FOUNDRY_LITE_TRANSFORM_SCHEDULER_CONTINUOUS", False),
        tenant_id=values.get("FOUNDRY_LITE_TENANT_ID", DEFAULT_TENANT_ID),
        actor_user_id=values.get("FOUNDRY_LITE_WORKER_ACTOR_USER_ID", "worker-transform-scheduler"),
        request_id=values.get("FOUNDRY_LITE_WORKER_REQUEST_ID", "req-worker-transform-scheduler"),
        evidence_path=_optional_path(values, "FOUNDRY_LITE_TRANSFORM_SCHEDULER_EVIDENCE_PATH"),
    )


def main(argv: list[str] | None = None) -> int:
    config: TransformSchedulerWorkerConfig | None = None
    try:
        parser = _parser()
        config = _config_from_args(parser.parse_args(argv))
        result = (
            run_transform_scheduler_daemon(config) if config.is_continuous else run_transform_scheduler_once(config)
        )
...
def _run_tick(
    ...
    *,
    ...
    return runtime.transforms.run_due(
...
def _summary(
    ...
    return TransformSchedulerWorkerResult(
        status="STOPPED",
...
def _run_id(run: object) -> str | None:
    if not isinstance(run, Mapping):
        return None
    for field in ("transformRunId", "runId"):
        value = run.get(field)
...
def _sleep_between_ticks(config: TransformSchedulerWorkerConfig, tick_number: int, *, max_ticks: int | None) -> None:
    if max_ticks is None or tick_number < max_ticks:
        time.sleep(max(0.0, config.tick_interval_seconds))


def _build_foundry(config: TransformSchedulerWorkerConfig) -> FoundryLite:
...
def _write_evidence(config: TransformSchedulerWorkerConfig, result: TransformSchedulerWorkerResult) -> None:
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="Run due Foundry-lite Transform schedules.")
...
def _config_from_args(args: argparse.Namespace) -> TransformSchedulerWorkerConfig:
    return replace(
        config_from_env(),
...
def _failure_json(exc: Exception, config: TransformSchedulerWorkerConfig | None) -> str:
...
def _failure_trace(config: TransformSchedulerWorkerConfig | None) -> Mapping[str, str]:
    ctx = _failure_context(config)
    if ctx is None:
        return {"adapter": "transform_scheduler_worker"}
    return {
        "tenant_id": ctx.tenant_id,
        "actor_user_id": ctx.actor_user_id,
        "request_id": ctx.request_id,
        "correlation_id": ctx.request_id,
        "adapter": "transform_scheduler_worker",
    }


def _failure_context(config: TransformSchedulerWorkerConfig | None) -> RequestContext | None:
    if config is None:
        return None
    try:
...
def _result_json(result: TransformSchedulerWorkerResult) -> str:
    return json.dumps(
        {
...
def _required_value(field: str, value: str) -> str:
    ...
    if not normalized:
        raise ValueError(f"transform scheduler worker requires {field}")
    return normalized


def _positive(value: int) -> int:
    if value <= 0:
        raise ValueError("transform scheduler worker numeric config must be positive")
    return value


def _positive_or_unbounded(value: int, field: str) -> int | None:
    if value < 0:
        raise ValueError(f"transform scheduler worker {field} cannot be negative")
