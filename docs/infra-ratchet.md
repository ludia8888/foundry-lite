...
- **P0h — Approval Execution Service + approved-as action linkage (shipped as the Approval-to-Action
  ...
  `foundry.aip.execute_approved_action(...)`, `POST /api/insights/reviews/{review_id}/execute-approved-action`,
  and `client.insights.reviews.executeApprovedAction(...)`. The service requires reviewer permission, requires
  ...
  definition, and re-reads the current target object version before claiming fresh execution. The API/SDK
  requires `Idempotency-Key`; the service persists the approval-execution claim in review metadata, lets
  same-key `executing` retries finish after action success, and rejects key reuse for a different request. Execution then
  ...
  after execution, finish-after-action recovery, durable review-to-action linkage, and CI runtime-lane wiring.
