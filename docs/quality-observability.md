...
| mixin 의존성 | `check_service_mixin_dependencies.py` | service mixin이 `CoreDependencies`에 선언되지 않은 숨은 `self.<attr>`에 기대는 문제 |
| mixin call graph | `check_mixin_call_graph.py` | service mixin 사이의 암묵적 cross-call이 순환하거나 depth/fan-out 한계를 넘는 문제 |
