...
cloud/Vault secret manager, OAuth authorization flow, SAP/NetSuite 전용 wizard packaging, scheduler UI는 아직 future다.
...
`error`, `retryable`을 유지한다. v1의 의도적인 future scope는 remote directory crawler, scheduler UI, managed
Debezium Connect operations, cloud secret manager, OAuth authorization flow, vendor-specific SAP/NetSuite packaged
source wizards다.
...
commit은 `startFirstSync`가 시작한 `ConnectorSyncWorkflow` data-plane에서만 생긴다. 현재 v1 future scope는
vendor-specific SAP/NetSuite adapter packaging, OAuth authorization flow, recurring scheduler, cloud/Vault secret
manager, cloud/Vault secret manager and secret rotation API, CDC/Debezium onboarding이다.
