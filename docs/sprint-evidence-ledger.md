...
| `S39-A2` | CDC event appends to raw changelog dataset | `VERIFY-CDC-STREAM-ARCHIVE`; `tests/integration/test_cdc_stream_archive.py` | Done for Debezium-shaped stream events |
| `S39-A3` | primary key and ordering metadata are visible in preview | `VERIFY-CDC-STREAM-ARCHIVE`; `tests/integration/test_cdc_stream_archive.py` | Done |
| `S39-A4` | delete event is standardized as `after=null` or tombstone policy | `VERIFY-CDC-STREAM-ARCHIVE`; `tests/contracts/test_debezium_cdc_adapter_contract.py` | Done for `after=null` delete envelopes |
