...
| `VERIFY-FULL-CI-GATE` | `env DOCKER_HOST=unix:///Users/isihyeon/.colima/default/docker.sock TESTCONTAINERS_DOCKER_SOCKET_OVERRIDE=/var/run/docker.sock pnpm --silent ci:gate` | PASS. Testcontainers preflight reached Colima, full pytest `698 passed`, coverage `95.87%`, flaky detector stable, layer/public callable coverage passed, OpenLineage/audit/outbox/MVP correctness/performance/trace/adapter-error/failed-mutation/runtime diagnostics/Playwright gates passed. |
...
| `VERIFY-SHADOW-REINDEX` | `DOCKER_HOST=unix:///Users/isihyeon/.colima/default/docker.sock TESTCONTAINERS_DOCKER_SOCKET_OVERRIDE=/var/run/docker.sock pnpm --silent quality:shadow-reindex`; `pnpm --silent quality:static` | PASS. Sprint 41 local verification: `quality:shadow-reindex` returned `38 passed`, covering fake/SQLite/Postgres object index/read/set repository contracts plus `tests/integration/test_shadow_reindex.py`. `quality:static` passed after schema revision `infra/schema_revisions/20260614_0001_shadow_reindex_index_versions.json`, proving schema drift, function length, module size, audit/transaction, tenant write, docs, and SDK gates still hold. PR/merge evidence is still pending for this local checkout. |
...
| Evidence id | Checkbox meaning | Git / test evidence | Current status |
|---|---|---|---|
| `S41-A1` | live Object Query keeps serving the active index while shadow rows are present | `VERIFY-SHADOW-REINDEX`; `tests/contracts/test_object_read_repository_contract.py::test_object_read_repository_contract_ignores_shadow_index_rows`; local PR/merge evidence pending | Done locally |
| `S41-A2` | validation success switches the active index version to the shadow namespace | `VERIFY-SHADOW-REINDEX`; `tests/integration/test_shadow_reindex.py::test_shadow_reindex_switches_after_validation_and_replays_action_edits`; local PR/merge evidence pending | Done locally |
| `S41-A3` | validation failure leaves the previous active index serving | `VERIFY-SHADOW-REINDEX`; `tests/integration/test_shadow_reindex.py::test_shadow_reindex_validation_failure_keeps_existing_active_index`; local PR/merge evidence pending | Done locally |
| `S41-A4` | object count/hash validation matches baseline before switch | `VERIFY-SHADOW-REINDEX`; `ObjectIndexValidationResult`; `object_index_stats`; local PR/merge evidence pending | Done locally |
| `S41-A5` | action edits replay into the shadow rebuild so current view is preserved | `VERIFY-SHADOW-REINDEX`; integration assertion keeps `status=APPROVED`, `operatorNote=Shadow validation proof`, and object version after switch; local PR/merge evidence pending | Done locally |
