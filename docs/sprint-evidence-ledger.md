...
| `VERIFY-FULL-CI-GATE` | `env DOCKER_HOST=unix:///Users/isihyeon/.colima/default/docker.sock TESTCONTAINERS_DOCKER_SOCKET_OVERRIDE=/var/run/docker.sock pnpm --silent ci:gate` | PASS. Testcontainers preflight reached Colima, full pytest `713 passed`, coverage `95.65%`, flaky detector stable across `3` repeated random/parallel runs with `713 passed`, layer/public callable coverage passed, OpenLineage/audit/outbox/MVP correctness/performance/trace/adapter-error/failed-mutation/runtime diagnostics/Playwright gates passed. |
...
| `S41-H2` | concurrent shadow promotions for the same object type cannot silently overwrite a previously validated active version | `VERIFY-ACTIVE-INDEX-POINTER`; `expected_previous_index_version`; `test_object_index_repository_contract_concurrent_first_pointer_switch_has_one_winner` | Done in pending concurrency PR |
