...
| `docs/action-types-parity-matrix.json`                    | `machine-registry` | Action Types v2 public-behavior parity matrix  | 공식 Palantir Action Type 동작과 현재 typed IR/EditPlan/atomic-commit 증거, 그리고 function/parameter/side-effect/log/revert/branch/interface/MCP gap을 `missing/foundation/partial/current`로 잠근다. |
