...
| T0-03 | Dataset DB commit vs object-storage manifest/files | Sprint 05, 36A | Partial | `S05-A1`, `S05-A2`, `S05-A3`, `S36A-A3`, and `VERIFY-DATASET-STORAGE-SPLIT-BRAIN` prove the local MVP split-brain path: storage promotion followed by DB file-row failure removes the promoted version directory and persists `orphan_cleanup` evidence on the FAILED sync run; committed DB versions with missing manifest or data file now raise operator-facing `committed_version_storage_missing`; the local adapter verifies manifest file URI, byte size, and content hash after write; `test_abort_cleanup_never_deletes_committed_manifest` proves an aborted later upload does not delete the earlier committed manifest or data file. Missing future proof: multipart/partial-object validation for S3-like production adapters. |
...
| T0-05 | Health check input candidate vs latest committed dataset | Sprint 08 | Partial | `test_dataset_health_check_reads_candidate_not_latest` proves an invalid duplicate-key candidate upload is rejected even when the latest committed version is valid, so health checks inspect the staged candidate rather than `latest`. Remaining hardening: persist `checked_manifest_hash` and compare it again immediately before final commit. |
| T0-06 | Schema compatibility TOCTOU | Sprint 08, 36A | Partial | `test_schema_compatibility_revalidates_if_latest_schema_changes` proves dataset finalization now takes the dataset version-allocation lock before schema compatibility validation, so schema validation and version allocation happen inside the same dataset lock window. Remaining hardening: persist `validated_against_schema_version_id` and add a richer integration race proof for production database isolation. |
...
| T0-09 | External before-commit success vs local action failure | Sprint 27 | Future | Current implementation-status says `mock_erp_simulator` records simulated writeback rows and does not call a real ERP. Real external success plus local failure must remain open until `COMPENSATION_REQUIRED` and reconciliation are first-class. |
| T0-10 | External timeout outcome unknown | Sprint 27 | Future | Same as T0-09. A real writeback adapter must treat timeout-after-write as `outcome_unknown`/reconciliation-required, not plain failure. |
...
| T0-34 | Action audit exposes sensitive values | Sprint 26, 34 | Covered | `test_action_audit_masks_sensitive_params` proves an action can update a sensitive `Order.margin` value while the durable `audit_events` before/after refs store `***MASKED***` instead of the raw previous value or raw patch value. Future ontology-level parameter sensitivity metadata remains a refinement, but current action audit evidence no longer exposes masked object properties. |
