...
| T0-09 | External before-commit success vs local action failure | Sprint 27, S53 | Partial | `VERIFY-DATA-PLATFORM-S53`, `quality:saga-reconciliation`, `test_external_success_local_failure_requires_compensation`, `test_compensation_is_idempotent`, `test_reconciliation_resolves_remote_success`, and `test_concurrent_reconciliation_has_one_winner` prove the simulated external-success/local-mutation-failure path becomes `compensation_required` action/writeback/audit evidence, same-key replay does not issue another writeback, and an operator-confirmed remote success can reconcile the local object mutation with one concurrent winner. Real external success plus local failure must remain open until a real writeback adapter, vendor remote lookup, compensation worker execution, and persistent reconciliation queue are first-class. |
| T0-10 | External timeout outcome unknown | Sprint 27, S53 | Partial | `VERIFY-DATA-PLATFORM-S53`, `quality:external-writeback`, `test_external_success_response_lost_becomes_outcome_unknown`, `test_outcome_unknown_is_not_blindly_retried`, and `test_action_repository_contract_reconciles_outcome_unknown_writeback_once` prove the simulated before-commit response-loss path becomes `outcome_unknown` action/writeback/audit evidence, same-key replay does not issue another writeback, and the stored writeback can be closed as `reconciled` only once after operator-provided remote-success evidence. A real writeback adapter, vendor remote lookup, and persistent reconciliation queue remain future under T0-09/S53. |
