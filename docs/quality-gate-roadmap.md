...
7. **Infra Ratchet 준수**: 새 production-style 인프라는 한 번에 하나씩 추가하고,
   ...
   인프라로 넘어간다.
8. **Infrastructure swapability 준수**: 교체 가능하다는 주장은 환경변수나 profile 이름으로
   공급자를 추측하는 방식으로 인정하지 않는다. port의 명시적 provider identity/capability,
...
`quality:mcp-rate-limits`는 Builder/Ontology/Governed Release의 POST/GET/DELETE endpoint와 tool 호출이
...
`quality:governed-release-mcp`는 로컬 대화형 release plane의 13개 tool을 고정한다: workspace 열기→격리 branch 생성→위젯 Builder handoff(별도 tool이 아님)→review inbox→사람 reviewer claim(보호 프로필은 작성자 self-claim 차단)→candidate 조회→작성자 exact source candidate/PR 게시→action 준비→decision→internal merge/Ontology activation→Pipeline `PROMOTED`→exact external deploy observation→status 흐름이다. exact terminal-success가 되면 `operationalCompletion`이 정상 운영 완료를 표시하며 rollback을 요구하지 않는다. 이와 별도로 exact 현재 OAuth reviewer에게만 서버가 `attestationPurpose=rollback_rehearsal`인 두 workflow root `completionCoordinates`를 제공할 수 있고, 그 좌표로 safe rollback→server-owned completion verification/`liveReadiness` attestation 리허설을 수행한다. `ui://foundry-lite/governed-release-v9-87ac4aeadd8c.html`은 MCP Apps `2026-01-26` JSON-RPC `postMessage` 초기화에서 ChatGPT same-window adapter와 parent host를 함께 탐색해 첫 유효 응답 source에 고정하고, 그 exact source로 tool-call/message/teardown을 처리하며 `window.openai`는 compatibility fallback으로만 유지한다. 빈 초기 tool output은 거부하고 bounded polling으로 늦은 서버 스냅샷을 복구하며 teardown에서 timer를 정리한다. 실제 proposal이 없는 workspace·branch-created·empty-inbox 카드는 상태 새로고침을 fail-closed해 sentinel ID로 `get_release_status`를 호출하지 않는다. 사람 승인은 stable OAuth session에 묶고, 요청마다 검증되는 MCP transport session과 짧은 access-token window의 회전은 같은 grant의 confirmation을 무효화하지 않는다. 각 tool의 top-level/compatibility `_meta` OAuth scope와 runtime `invalid_token`/`insufficient_scope` re-link challenge도 함께 고정한다. 외부 OIDC 계약은 명시적 HTTPS authorization server/public base, issuer 일치, exact scalar resource audience, server-bound app id, configurable client/session/human-grant claim, raw `sid`를 숨긴 stable issuer-session hash, 외부 세션의 local ledger 비조회와 local 세션 revocation 유지를 회귀 검증한다. app-only visibility는 준수 host UI 계약이지 물리적 click의 암호학적 증명이 아니다. deterministic idempotency key와 bounded durable recovery는 응답 유실·재로드 exact replay를 fail-closed로 처리한다. mutation 전 admission/precondition failure로 `knownNotCommitted`와 `safeToRetry`가 모두 남은 exact failed run에만 consumed receipt 재발급→fresh app-only confirmation→quota→failed-to-running CAS→새 fenced attempt를 허용한다. tokenless replay나 succeeded/running/outcome-unknown/분류 불가 실패는 reopen하지 않는다. 실패한 candidate deploy가 live가 되지 않은 `deployment_failed`에서는 현재 Render live deploy를 보존하고 internal Pipeline promotion만 안전 복구하며 provider rollback을 보내지 않는다. 보호 프로필에서는 Ontology apply/rollback/proposal execute와 Pipeline proposal execute/deploy가 exact running Governed Release action의 tenant/actor/app/client/OAuth session/MCP session/tool/permission/binding/attempt/proposal 증거를 다시 읽고, direct REST/SDK 우회는 durable deny audit와 함께 fail-closed한다. local/demo/test direct admin/IaC 호환은 유지하며, 이 경계는 모든 플랫폼 mutation·raw DB·hostile in-process caller까지 포괄하는 universal sandbox가 아니다. Pipeline `PROMOTED`는 내부 serving version 승격이지 Foundry-lite 앱 인프라 배포가 아니다. 2026-08-12 hosted ChatGPT + public HTTPS + local Authorization Code OAuth의 workspace open→exact branch create→Builder handoff는 transport-session/access-token 회전을 포함해 live 확인했다. 후속 v9 read-only inbox 호출은 actual v9 iframe, `empty-inbox` 렌더링, 8개 action disabled 상태와 새 탭 재접속 복구를 live 확인했다. 실제 production public deployment, external IdP interoperability run, GitHub PR merge, Render deploy/rollback, Foundry-lite app Kubernetes/Vercel deploy는 여전히 live proof 밖이다.

검토 정책은 **배정된 사람의 명시적 승인**을 강제하되 보호 프로필에서는 작성자와 검토자의 IdP principal이 달라야 한다. 작성자의 self-claim·승인을 차단하는 회귀 경로와, 미배정·다른 담당자·stale 후보를 계속 차단하는 경로를 함께 검증한다. GitHub ruleset이 별도 리뷰어를 요구하면 provider 정책은 추가 조건으로 남는다.

`test_governed_release_widget_uri_is_content_addressed`는 embedded HTML SHA-256 앞 12자리와 현재 resource URI suffix를 직접 비교한다. 위젯 HTML을 바꾸고 URI를 그대로 두면 gate가 실패하므로 hosted ChatGPT가 같은 URI의 오래된 자산을 재사용하는 회귀를 차단한다. 이전 `v2`·`v3`·`v4` URI는 기존 대화의 resource read 호환만 유지하고 새 `tools/list`/`resources/list`에는 노출하지 않는다.

v9 gate는 정상 스냅샷에서 secret-free recovery coordinate만 표준 `ui/update-model-context`로 저장하고, 재로드에서는 그 좌표로 read-only workspace/inbox/status tool 중 하나만 exact 호출하는지 검증한다. `window.openai` global이 전혀 없어도 bounded wait가 시작되고, 좌표가 끝까지 없으면 `aria-busy=false`인 명시적 복구 안내로 전환해야 한다. Hosted ChatGPT live QA에서는 새 대화의 v8 `empty-inbox`와 같은 URL의 새 탭 재접속을 확인했고, 5초 skeleton이 23초 이내 서버 스냅샷으로 복구되며 durable tool quota가 read-only 1회만 증가했다. 실제 pending Ontology와 `in_review` Pipeline proposal의 fresh/reload도 확인했다. Pipeline의 비관여 OAuth 사용자는 claim·decision·merge·deploy·rollback이 모두 disabled였고, reload timeline은 proposal 제출·배정 2건만 포함했으며 version/delivery/protected-action audit는 변하지 않았다. 실제 nested iframe의 상태 새로고침 버튼도 hosted에서 클릭해 새 request ID와 서버 snapshot 완료 안내를 확인했고, 그 fixed window의 tool quota는 read-only 1회만 증가했다. 이어 exact GitHub repository metadata와 deliberately invalid non-secret token을 사용한 작성자 경로에서는 `GitHub 후보 PR 게시`만 enabled이고 reviewer/merge/deploy/rollback은 disabled임을 확인했다. prepare/publication은 호출하지 않았으며 reload 뒤에도 one-read quota, pending proposal, zero delivery/protected audit, zero open PR이 유지됐다. 이 live slice는 publication 권한 projection의 증거이지 실제 GitHub publication, 외부 IdP 상호운용, merge, Ontology activation, Render deploy/rollback의 증거가 아니다.
...
| G24  | `check_infrastructure_swapability.py`   | §4.3 20-27                                                                      | provider-neutral application module의 공급자 literal/profile-name 추측/legacy provider-shaped policy field 0, explicit provider identity/live capability, factory registry, alternate-provider regression 존재                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | 이름만 port인 provider lock-in 차단                        |
...
`tests/sdk/request_contract.mjs`는 browser SDK를 실제 import해 322개 frontend route surface의
...
| AI FDE governed platform contract | `quality:ai-fde` | 9개 mode의 canonical catalog union 69개가 실제 Builder MCP JSON-RPC `tools/call`에서 빠지거나 native tool adapter가 invoking-user permission을 우회하고 durable 성공 증거를 남기지 못하거나, 49개 read가 불필요한 승인을 요구하거나 20개 mutation이 challenge → 별도 human bearer 또는 app-only GPT widget 승인 → 짧은 수명의 1회용 receipt를 거치지 않거나, private `approve_builder_mutation`가 모델에 보이거나 shared Builder tool quota를 우회하거나 exact recovery가 quota를 다시 소비하거나, widget token/receipt가 `structuredContent`/`content`에 노출되거나, raw token이 DB에 저장되거나, actor/app/client/OAuth session/MCP session/Origin/challenge/request binding·pending replay token rotation·expiry·single-use가 깨지거나, 승인 응답 유실 뒤 exact app retry가 같은 미소비 receipt를 복구하지 못하거나, mutation commit 뒤 응답 유실 시 새 승인 없이 exact durable replay를 돌려주지 못하거나, widget이 tool/mode/workspace/request hash와 전체 secret-redacted 입력 및 실제 저장 영향을 보여주지 않거나 exact original call 대신 다른 입력을 재호출하거나, inactive MCP session이 Builder resource를 읽거나, lazy search가 server-owned catalog 밖 tool을 활성화하거나, Ontology/Pipeline branch를 벗어나 쓰거나, exact Builder audience·OAuth app scope·Origin·재전송·structured content 계약이 깨지거나, Pipeline static test proof를 실제 data execution으로 잘못 보고하거나, Pilot의 seed/branch/OSDK/React/CI bundle idempotency 및 API/SDK/UI/runtime evidence가 drift하는 문제 차단 |
...
| Governed Release MCP contract | `quality:governed-release-mcp` | 13개 tool의 workspace open→isolated branch create→Builder handoff→review inbox/claim→candidate→author-confirmed exact source candidate/PR publish→prepare/decision→internal merge/Ontology activation→Pipeline `PROMOTED`→external deploy observation→status 흐름과, rollback 없이 끝나는 정상 `operationalCompletion`, 별도 rollback rehearsal→server-owned completion verification/`liveReadiness` attestation을 구분한다. `ui://foundry-lite/governed-release-v9-87ac4aeadd8c.html`의 MCP Apps `2026-01-26` 표준 `postMessage` initialize/tool-call/message/teardown primary bridge 및 `window.openai` compatibility fallback, empty-initial-output rejection and bounded snapshot recovery/teardown cleanup, no-status-read before a real proposal exists, stable OAuth-session confirmation across rotating MCP transport sessions and access-token windows, tool별 top-level/compatibility OAuth scope, safe runtime re-link challenge, 사람 Authorization Code principal, app-only tool visibility, MCP App resource MIME/URI, target-schema validation, exact 현재 OAuth reviewer에게만 제공되는 server-owned `completionCoordinates`, verify completion의 closed workflow-run-id schema와 server-owned collector dispatch, `_meta`-only one-time widget token, hash-only durable receipt, actor/app/client/session/Origin/arguments binding, deterministic caller-idempotency exact replay, bounded durable recovery, 배정된 사람 reviewer(보호 프로필은 작성자 self-review 차단), stale proposal 차단 중 하나가 끊기거나 Builder plane에 release mutation이 다시 노출되는 문제를 차단한다. `deployment_failed`에서 실패한 candidate가 live가 되지 않았는데 provider rollback을 보내거나 application rollback target을 노출하는 회귀도 차단한다. known-not-committed pre-mutation failure만 fresh app-only confirmation과 quota 뒤 CAS fenced attempt로 reopen하며 tokenless/succeeded/running/outcome-unknown/unclassified 상태는 reopen하지 않는다. 보호 프로필의 direct Ontology apply/rollback/proposal execute와 Pipeline proposal execute/deploy가 exact running release action 증거 없이 성공하거나 durable deny audit가 사라지는 회귀도 차단한다. local/demo/test direct admin/IaC 호환은 유지한다. app-only는 physical-click crypto-proof가 아니며 이 gate는 모든 platform mutation/raw DB/hostile in-process caller의 universal sandbox가 아니다. Pipeline `PROMOTED`는 내부 serving 승격이다. hosted ChatGPT public HTTPS + local OAuth의 exact branch create/Builder handoff는 2026-08-12 실증했다. external IdP/DCR interoperability·GitHub PR merge·Render deploy/rollback·Foundry-lite app Kubernetes/Vercel deploy는 미검증 경계다. |
| MCP durable admission contract | `quality:mcp-rate-limits`; `quality:mcp-rate-limits-live` | 세 MCP plane의 POST/GET/DELETE 또는 tool quota가 누락되거나 tool/session 이름 회전으로 우회되거나, exact replay가 quota를 다시 소비하거나, fixed-window 동시 갱신·tenant RLS·audit/outbox·`Retry-After` 계약이 깨지는 문제 차단 |
