"""Change constructors for ontology migration planning."""
...
def blocked_primary_key_changed(api_name: str, previous: str, candidate: YamlObject) -> OntologyMigrationChange:
    ...
    return _blocked(
        ...
        True,
        details={"previous": previous, "next": required_str(candidate, "primaryKey")},
    )
...
def blocked_property_type_change(
    ...
    return _blocked(
        ...
        True,
        details={"previous": current["data_type"], "next": required_str(candidate, "type")},
    )
...
def blocked_link_endpoint_changed(
    ...
    return _blocked(
        ...
        details={
            ...
            "next": {"from": required_str(candidate, "from"), "to": required_str(candidate, "to")},
        },
    )
...
def blocked_link_backing_changed(
    api_name: str,
    previous: LinkTypeBacking,
    next_backing: LinkTypeBacking,
) -> OntologyMigrationChange:
...
def blocked_parameter_type_changed(
    ...
    return _blocked(
        ...
        True,
        details={"previous": required_str(current, "type"), "next": required_str(candidate, "type")},
    )
