"""Application service helpers for insight review service workflows."""
...
class InsightReviewService(CoreService):
        ...
        *,
        ...
        with self.engine.begin() as conn:
            ...
            if existing is not None:
                ensure_create_replay_matches(existing, record)
            row = existing or self._require_row(conn, ctx, record.review_id)
            if existing is None:
                self._audit(conn, ctx, "created", row, after_ref=review_payload(row))
    ...
    def list_reviews(
        ...
        *,
        ...
        return {"items": [review_payload(row) for row in rows]}
    ...
    def assign_review(
        ...
        *,
        ...
        with self.engine.begin() as conn:
            ...
            self._audit(conn, ctx, "assigned", row, before_ref=review_payload(before), after_ref=review_payload(row))
            return review_payload(row)
    ...
    def decide_review(
        ...
        *,
        ...
        with self.engine.begin() as conn:
            ...
            self._audit(conn, ctx, parsed, row, before_ref=review_payload(before), after_ref=review_payload(row))
            return review_payload(row)
