...
type WorkshopPage = { id:string; pageId:string; name:string; isDefault:boolean; sections:WorkshopSection[] };
type WorkshopApp = { name:string; purpose:string; version:number; theme:{preset:string;brandName:string;logoText:string}; shell:{navigation:string}; presentation?:{locale?:string;objectTypeNames?:Record<string,string>;propertyNames?:Record<string,string>;actionNames?:Record<string,string>;statusLabels?:Record<string,{label:string}>;booleanLabels?:{trueLabel:string;falseLabel:string;emptyLabel:string};chrome?:{workspaceLabel:string}}; product?:{capabilityGroups?:Array<{name:string;pageIds:string[]}>;trustCenter?:{approvalStatement?:string}}; pages:WorkshopPage[] };
...
function statusLabel(raw:unknown) { const key=String(raw ?? "");return workshop.presentation?.statusLabels?.[key]?.label ?? (key.replaceAll("_"," ").toLowerCase().replace(/^./,(letter)=>letter.toUpperCase()) || "상태 미지정"); }
function displayValue(raw:unknown) { if(raw===null||raw===undefined||raw==="")return workshop.presentation?.booleanLabels?.emptyLabel ?? "정보 없음";if(typeof raw==="boolean")return raw?(workshop.presentation?.booleanLabels?.trueLabel ?? "예"):(workshop.presentation?.booleanLabels?.falseLabel ?? "아니요");if(Array.isArray(raw))return `${raw.length}개 항목`;if(typeof raw==="object")return "상세 정보 있음";return String(raw); }
function itemTitle(item:{objectType:string;objectId:string;properties:object}) { const properties=item.properties as Record<string,unknown>;const typeName=workshop.presentation?.objectTypeNames?.[item.objectType] ?? "업무";return String(properties.name ?? properties.title ?? `${typeName} #${item.objectId.slice(-4)}`); }
...
export default function App() {
  ...
  function objectRows() { return <div className="object-list">{filtered.length?filtered.map((item)=><button type="button" key={`${item.objectType}:${item.objectId}`} className={item.objectId===(selected?.objectId ?? "")?"object-row selected":"object-row"} onClick={()=>setSelectedId(item.objectId)}><strong>{itemTitle(item)}</strong><small>{statusLabel(item.properties.status ?? "확인 필요")}</small></button>):<div className="empty"><strong>표시할 업무가 없습니다</strong><p>검색 조건을 바꾸거나 새 업무를 시작해 보세요.</p></div>}</div>; }
  function detail() { return selected?<details open><summary>업무 정보 자세히 보기</summary><dl>{recordFields.map((field)=><div key={field.apiName}><dt>{field.displayName}</dt><dd>{field.apiName==="status"?statusLabel(property(selected,field.apiName)):displayValue(property(selected,field.apiName))}</dd></div>)}</dl></details>:<p className="empty">목록에서 업무를 선택하세요.</p>; }
  function statusTracker(property:string) { return <div className="tracker">{groupBy(filtered,property).map(([name,items])=><div className="track" key={name}><span>{statusLabel(name)}</span><strong>{items.length}<small>건</small></strong></div>)}</div>; }
  function kanban(property:string) { return <div className="kanban">{groupBy(filtered,property).map(([name,items])=><section className="lane" key={name}><h3>{statusLabel(name)}<small>{items.length}</small></h3>{items.slice(0,30).map((item)=><button type="button" key={item.objectId} onClick={()=>setSelectedId(item.objectId)}>{itemTitle(item)}</button>)}</section>)}</div>; }
  ...
  const capability=workshop.product?.capabilityGroups?.find((group)=>group.pageIds.includes(currentPage?.pageId ?? ""))?.name ?? "오늘의 업무 흐름";
  return <main className="app-shell" style={themeVariables(workshop.theme.preset)}><aside className="side"><div className="brand"><span>{workshop.theme.logoText}</span><div><strong>{workshop.theme.brandName}</strong><small>{workshop.presentation?.chrome?.workspaceLabel ?? "업무 운영 공간"}</small></div></div><nav aria-label="업무 화면">{workshop.pages.map((page)=><button type="button" key={page.id} className={page.id===currentPage?.id?"active":""} onClick={()=>setPageId(page.id)}>{page.name}</button>)}</nav><footer>{workshop.product?.trustCenter?.approvalStatement ?? "권한과 승인 규칙이 적용됩니다"}</footer></aside><div className="workspace"><header className="context"><div><small>{capability}</small><h1>{currentPage?.name}</h1><p>{workshop.purpose}</p></div><div className="pulses"><span>필터 {statusFilter?"1개":"전체"}</span><span>{selected?"업무 선택됨":"선택 전"}</span><span className="live">최신 상태</span></div></header><div className="decision"><span>1. 업무 선택</span><span>2. 다음 업무 실행</span><span>3. 변경 근거 보존</span></div><div className="canvas">{currentPage?.sections.map((section)=><section className={`section ${section.layout}`} style={{gridColumn:`span ${section.span || 12}`}} key={section.id}><h2>{section.title}</h2><div className="widgets">{section.widgets.map((widget)=><article className="widget" data-workshop-widget={widget.kind} key={widget.id}><header><strong>{value(widget.config,"title","업무 보기")}</strong></header><div className="widget-body">{renderWidget(widget)}</div></article>)}</div></section>)}</div>{policies.length?<details className="policy"><summary>적용 중인 업무 규칙</summary>{policies.map((policy)=><article key={policy.name}><strong>{policy.name}</strong><p>{policy.statement}</p></article>)}</details>:null}{message?<p role="status">{message}</p>:null}</div></main>;
}
"""
...
.brand strong{font-size:15px}.brand small{font-size:10px;letter-spacing:0;text-transform:none}
.side nav button{border-radius:11px;padding:11px 12px;font-size:13px}
.context{border-bottom:0;padding-bottom:14px}.context small{font-size:10px;letter-spacing:0;text-transform:none}.context h1{font-size:30px}
.decision{display:grid;grid-template-columns:repeat(3,1fr);margin:0 28px;overflow:hidden;border:1px solid #dde3e9;border-radius:12px;background:#f7f9fb}
.decision span{border-right:1px solid #dde3e9;padding:10px 12px;color:#657386;font-size:11px;font-weight:700}.decision span:last-child{border-right:0}
.canvas{padding-top:20px}.widget>header strong{font-size:13px}.object-row strong{font-size:12px}dl div{font-size:12px}
@media(max-width:900px){.app-shell{grid-template-columns:1fr}.decision{grid-template-columns:1fr;margin:0 16px}.decision span{border-right:0;border-bottom:1px solid #dde3e9}.decision span:last-child{border-bottom:0}}
"""
