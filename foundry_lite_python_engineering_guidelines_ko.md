...
| ConnectorAdapter | CSV/local, REST secretRef refresh, webhook, Debezium wrapper proof | durable registry, retry workers, SaaS connector | sync run lifecycle과 cursor/checkpoint 의미가 동일해야 한다. |
| SecretProvider | local env, webhook signing key, REST connector credential secretRef | Vault/cloud secret manager, previous/current rotation registry | secret 값은 audit/log/error/operator evidence에 raw로 남지 않는다. |
| AuthProvider/PolicyAdapter | local/demo/header-trust + local JWT/OIDC human/M2M verification + RBAC + production unsafe-profile guard | live OIDC/SSO, service-account registry, ABAC/CBAC | permission decision과 audit deny 형식이 동일해야 한다. |
| PrivacyTransform | tenant-scoped pseudonym, basic anonymization, local text PII redaction, protected in-memory reversible mapping proof, production-to-nonprod replication policy proof, source-to-anonymized target dataset lineage artifact | durable environment replication workflow, durable/encrypted protected mapping backend, runtime DB lineage/outbox/OpenLineage transport, erasure lifecycle | privacy output은 replay 가능해야 하고 tenant 간 pseudonym을 연결하지 않는다. Reversible mapping evidence는 원본값을 노출하지 않는다. Production data는 민감 필드 보호 plan 없이 staging/analytics/AI 실험 환경으로 복제하지 않는다. Dataset lineage/OpenLineage artifact에도 raw PII 값을 싣지 않는다. |
| ErasureLifecycle | deletion request redacted evidence, tenant-scoped subject resolution, idempotent erasure manifest, backup-retention pending state, audit-minimization action, search rebuild exclusion proof | durable erasure request table/API/workflow, ObjectStore/SearchAdapter/materialization/DLQ/backup/audit executors, KMS/cloud crypto-shredding | right-to-erasure 증거는 raw subject 값을 운영 증거에 노출하지 않는다. 같은 요청/idempotency key replay는 같은 manifest를 만든다. 다른 tenant의 같은 subject 값은 절대 같이 지우지 않는다. Backup retention 때문에 즉시 지우지 못하는 항목은 pending 상태와 key ref로 남긴다. |
| AIEvidence | object explain `propertyLineage`, immutable evidence reference, source-span redaction, model/prompt/extractor version pinning | durable evidence table, LLM extraction executor, insight evidence viewer, model diff UI | AI/insight claim은 source object와 source dataset version, model/prompt/extractor version, human review status 없이 생성하지 않는다. 재처리는 기존 evidence를 덮어쓰지 않고 새 revision을 만든다. Masking된 source span은 raw quote를 노출하지 않는다. |
| FrontendSDK | generated SDK request wrapper, request id, tenant/user/role context header, typed frontend error, retryability display | login/session UI, automatic retry/backoff, cursor helper, duplicate-click lock, stale-version conflict UI, permission-denied masking UX | 프론트는 raw DB/vendor SDK를 직접 호출하지 않고 public API/generated SDK를 사용한다. 화면의 에러는 request id, error code, retryability를 잃지 않아야 한다. Action 실행은 expected object version과 idempotency key를 함께 보내야 한다. |
