...
- Schema revision guard 게이트(`scripts/quality/check_schema_revision_guard.py`)는 `libs/foundry_lite/infrastructure/schema.py`의 SQLAlchemy metadata fingerprint가 `infra/schema_revisions/*.json` 최신 snapshot과 다르면 release gate를 차단한다. 현재 브랜치는 Alembic baseline migration과 fresh-DB metadata parity test를 갖지만, multi-step upgrade/rollback 운영은 아직 future scope다. DB 모양을 바꾸는 변경은 schema revision snapshot과 migration parity를 같은 변경에 포함해야 한다.
