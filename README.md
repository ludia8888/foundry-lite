...
S60의 현재 slice는 object explain `propertyLineage`, `EvidenceReference`, `EvidenceSourceSpan`, `build_insight_claim_payload`, `build_llm_extraction_evidence`, `revise_evidence_reference`, `quality:ai-evidence`로 property-level source coordinates, evidence-object-required insight claim, model/prompt/extractor version-pinned LLM evidence, immutable evidence revision, masked source-span redaction을 증명하는 범위입니다. S61의 현재 slice는 generated SDK의 `FoundryLiteApiError`, `createRequestId`, `requestContextHeaders`, `retryWithBackoff`, `collectCursorPages`, `createInFlightActionLock`, `createFoundryLiteOsdkClient`, `createFoundryLiteOntologyIndex`, `createSessionTokenProvider`, `pollFoundryLiteOperation`, `streamFoundryLiteOperationEvents`, `getObjectType`, `getActionType`, `adminCapabilityView`, `adminOperationsBoard`, `adminReadinessScreen`, `adminTaskView`, `adminTaskPlanScreen`, `groupAdminCapabilities`, `actionLockKey`, `classifyFoundryLiteError`, admin capability `executionSurface`/`canStartFromBrowser`/`requiresApproval`/`operatorChecklist`/`blockingReason` preflight fields, `requiresIdempotencyKey` surface의 caller-supplied `idempotencyKey`/`MISSING_IDEMPOTENCY_KEY` fail-fast, named SDK namespaces, Ontology-style object/action/media OSDK facade, large ontology registry lookup/live-catalog search/action grouping/dynamic-only drift hint, React live ontology catalog workspace model, session token provider, bounded operation polling, fetch-based operation event streaming helper, React cursor pagination and start-and-poll job screen helpers, React operation event stream state helper, React admin console launch model, admin task plan/operations board screen model, Source onboarding recipe/hook, Source Wizard recipe/hook, Generic REST connector onboarding recipe/hook, typechecked `@foundry-lite/sdk/screen-recipes` core product-screen builders, screen-level SDK recipe contract, ontology catalog, dataset list/versions/preview/inspect/qualityChecks/qualityResults, operations admin overview/task plan/lineage get, bounded outbox publish, Iceberg maintenance `planReadOnly`/`plan`, source CSV/batch/webhook/CDC/media/REST onboarding, connector connection/resource onboarding, AIP Builder validate/run, AIP Agent run, AIP eval run/release promote, media bytes upload/commit/process/index/search/reference, Pipeline Builder SQL transform register/run, `docs/frontend-api-sdk-surface-matrix.json`의 route/helper proofClass/request-contract mapping, `tests/sdk/request_contract.mjs`의 114개 browser SDK method/path/query/header/body/typed-error proof와 25개 SDK helper-runtime proof, Web Operations의 named SDK-only 호출, request id/error/retryability 표시, `quality:frontend-backend-surface`, `quality:sdk-request-contract`, `quality:frontend-foundation`을 증명하는 범위입니다.
...
| Production connector registry/retry workers | 부분 구현, Generic REST connector registry/onboarding API/SDK와 registered resource sync fingerprint guard는 active. Managed retry workers, SAP/NetSuite-specific adapters, OAuth onboarding, recurring scheduler, cloud/Vault secret manager, and cloud/Vault secret manager and secret rotation APIs are future.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| [docs/sdk-frontend-cookbook.md](docs/sdk-frontend-cookbook.md)                                             | 프론트 개발자가 SDK로 Dataset/Object/Media/AIP/Pipeline/Admin 화면을 조립하는 예제         |
